...
exports.proto = function(Linker) {
	...
	proto.add = deprecate.function(
		_addClient,
		'use `linker.client` instead of `linker.add`'
	);
	proto.addClient = deprecate.function(
		_addClient,
		'use `linker.client` install of `linker.addClient`'
	);
	proto.parseMethodKey = deprecate.function(
		proto.parseAction,
	...
	proto.bindFlow = deprecate.function(function(name, handler) {
		...
		if (typeof name == 'object') _.each(name, bindFlow);
		else if (typeof name == 'function') return bindFlow(name, name.name);
		else return bindFlow(handler, name);
	...
	proto.loadFlow = deprecate.function(function(name, pkgpath, module) {
		...
		function loadFlow(name, pkghandler, module) {
			...
			debug(
				'loadflow, file: %s, name: %s, module: %s',
				pkghandler,
				name,
				module
			);
			let handler = module
				? module.require(pkghandler)
				: require(pkghandler);
		...
		} else return loadFlow(name, pkgpath, module);
...
function _addClient(name, options) {
	...
	if (typeof name == 'object') _.each(name, addClient);
	else addClient(options, name);
}
