	...
	runFlows_: function(flowList, runtime, flowIndex)
	{
		...
		var promise = defer.promise
				...
				function(err)
				{
					...
					// 将错误信息转化为Error对象
					if (options.anyToError && !(err instanceof Error))
					{
						var errType = typeof err;
						var originalError = err;
						if (err && errType == 'object')
						{
						...
						{
							err = new Error(originalError || 'CLIENT_LINKER_DEFERT_ERROR');
						}
					...
					// 方便定位问题
					err.fromClient || (err.fromClient = runtime.methodKey);
