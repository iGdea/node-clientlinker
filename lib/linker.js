...
Linker.prototype =
{
	...
	run: function(action, query, body, callback, options)
	{
		...
		return self.newRuntime(action, query, body, options)
			.then(function(runtime)
			{
				runtime.data.source = 'run';
	...
	newRuntime: function(action, query, body, options)
	{
		var self = this;

		return self.parseMethodKey(action)
			.then(function(data)
			{
				if (data.client)
				{
				...
				else
				{
					...
					err.CLIENTLINKER_METHODKEY = action;
					throw err;
				}
			});
	},
	...
	// 解析action
	parseMethodKey: function(action)
	{
		return this.clients()
			.then(function(list)
			{
