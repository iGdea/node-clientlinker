...
var proto = Linker.prototype =
{
	...
	addClient: function(clientName, options, notOverride)
	{
		...
		if (typeof clientName == 'object')
		{
			...
			{
				result[name] = self._addClient(name, options, notOverride);
			});
		...
		{
			return self._addClient(clientName, options, notOverride);
		}
	},
	...
	loadFlow: function(pkgpath, module)
	{
		...
		if (STATIC.sysflows[pkgpath] && !module)
		{
			...
			return self._loadFlow(p, null, pkgpath, true);
		}
	...
	_bindFlow: function(flowName, handler)
	{
		...
		if (typeof handler == 'function')
		{
			...
			var flow = self.flows[flowName] = new Flow(flowName, handler);

			if (typeof flow.init == 'function')
			{
				self._addInitProcessWorkflow(function()
					{
						return flow.init(self);
					});
			}
	...
	getFlow: function(name)
	{
		...
		if (STATIC.sysflows[name])
		{
			...
			this._loadFlow(p, null, name, false);
	...
	_addInitProcessWorkflow: function(handler)
	{
		var self = this;

		self._initProcessPromise = self._initProcessPromise
			.then(handler);
	},
	// 标准输入参数
