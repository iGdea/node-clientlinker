...
_.extend(FlowsRun.prototype,
{
	...
	run_: function(callback)
	{
		...
		try {
			var ret = flow.run(runtime, callback, self);
			if (isPromise(ret)) ret.then(callback.resolve, callback.reject);
		}
		catch(err)
		{
			callback.reject(err);
	...
	callbackDefer_: function()
	{
		...
		function next()
		{
			if (!nextPromise)
			{
				nextPromise = self.run_(self.callbackDefer_());
			}

			return nextPromise;
		}

		function nextAndResolve()
		{
			next().then(callback.resolve, callback.reject);
		}
		...
		function resolveHanlder()
		{
			debug('flow resolve:%s', callback.flow && callback.flow.name);
			resolve.apply(null, arguments);
		}

		function rejectHanlder()
		{
			debug('flow reject:%s', callback.flow && callback.flow.name);
			reject.apply(null, arguments);
		}

		callback.promise		= promise;
		callback.resolve		= resolveHanlder;
		callback.reject			= rejectHanlder;
		callback.next			= next;
		callback.nextAndResolve	= nextAndResolve;
		callback.timing			= {};
		callback.flow			= null;
